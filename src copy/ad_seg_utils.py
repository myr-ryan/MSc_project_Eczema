...
def load_model(dir, classes=None, weights=None):
    ...
    loaded_model = keras.models.load_model(dir, custom_objects=dependencies, compile=False)
...
def run_gt_image_combination_only_remove_background(file_name, image_dir, mask_dir, output_dir, refno=None, visno=None):
    ...
    whole_img_name = refno + "_vis-" + visno + "_" + file_name.split(".")[0] + ".jpg"
    plt.imsave(os.path.join(whole_dir, whole_img_name), image_cp)
    ...
    for cntr in contours:
        ...
        if crop_filter(area):
            ...
            for img in img_crops:
                # weekno = file_name.split(".")[0]
                # weekno = weekno[6:]
                crop_file_name = refno + "_vis-" + visno + "_" + file_name.split(".")[0] + "_crop-" + str(
                    num_crops) + ".jpg"
                plt.imsave(os.path.join(crop_dir, crop_file_name), img)
...
def listdir_nohidden(path, is_train=True, PERTURBATION=None, SEVERITY=None):
...
#         print("??")
        if any("_aug" in string for string in file_list):
            file_list = [f.replace('_'+PERTURBATION+'_i'+str(6 - int(SEVERITY))+'_aug', '') for f in file_list]
            file_list = sorted(file_list)
            # print(file_list)
            temp_list = []
            for f in file_list:
                if '.jpg' in f:
                    temp_list.append(f.replace('.jpg', '_'+PERTURBATION+'_i'+str(6 - int(SEVERITY))+'_aug.jpg'))
                elif '.JPG' in f:
                    temp_list.append(f.replace('.JPG', '_'+PERTURBATION+'_i'+str(6 - int(SEVERITY))+'_aug.JPG'))
                elif '.png' in f:
                    temp_list.append(f.replace('.png', '_'+PERTURBATION+'_i'+str(6 - int(SEVERITY))+'_aug.png'))
                elif '.PNG' in f:
                    temp_list.append(f.replace('.PNG', '_'+PERTURBATION+'_i'+str(6 - int(SEVERITY))+'_aug.PNG'))
            file_list = temp_list
            # print(file_list)
...
class Dataset:
    ...
    def __init__(
        ...
        self.images_ids = listdir_nohidden(images_dir, is_train, PERTURBATION, SEVERITY)
#         print(self.images_ids[2])
        self.masks_ids = listdir_nohidden(masks_dir, is_train, PERTURBATION, SEVERITY)
